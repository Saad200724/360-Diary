# 📔 360 Diary - 2026
> "Consistency is the bridge between goals and accomplishment."
<img width="1337" height="550" alt="image" src="https://github.com/user-attachments/assets/3e06f72d-7d23-4b7a-8e0f-372bf80551e6" />
##Terminal Based 360 Journal
Welcome to my 2026 digital journal. This repository serves as a daily log of my progress, challenges, and milestones as a developer, entrepreneur, and student.

## 👤 Identity & Roles
- **Name:** Saad Bin Tofayel (Tahsin)
- **Leadership:** Founder & CEO of **ZnForge** (Web Development Agency)
- **Community:** Founding Secretary General & President of **SCPSC Cyber Hub**
- **Focus:** Full-Stack Web Development (Transitioning from No-Code to Pro-Code)
- **Academic:** SSC Candidate 2026

## 🎯 2026 Mission
- [ ] **The 2026 Streak:** Maintain a daily contribution graph from Jan 20th to Dec 31st.
- [ ] **ZnForge Expansion:** Scale my agency to provide high-end web solutions.
- [ ] **Mastering Code:** Transitioning deeper into full-stack development via Programming Hero.
- [ ] **Academic Excellence:** Balancing tech leadership with SSC exam preparation.

## 📅 Daily Log (January 2026)

| Date | Key Highlights & Progress | Status |
| :--- | :--- | :--- |
| 20 Jan | Initialized '360 Diary', configured GitHub PAT, and started the streak. | ✅ Completed |
| 21 Jan | Decided to Fuck leisure, just be active till got a glorifying acheivement | ✅ Completed |
| 22 Jan | Will complete the website of Oxygen Robotics R&D and complete ecommerce pet shop project by Friday 6pm with screen recording | ⏳ Pending |
| 23 Jan | complete meow and ask for the due | ⏳ Pending |
| 24 Jan | be patient stop fb | ⏳ Pending |
| 25 Jan | complete general math |⏳ Pending |
| 28 Jan | Have improved English listening skills than before and heared approx. 15 hour of video in English Dubbed| ✅ Completed |
| 31 Jan | War against myself, want to be the best version from today and started regenerating myself | ⏳ Pending |
| 2  Feb | War against myself, want to be the best version from today and started regenerating myself | ⏳ Pending |
| 4  Feb | exam pera | ⏳ Pending |



---

## 🛠 Tech Environment
- **Operating System:** Ubuntu
- **Terminal:** Bash
- **Source Control:** Git / GitHub
- **Hardware:** HP Laptop 14-bs0xx

---
*Driven by logic, fueled by passion. Building the future at ZnForge.*# 360-Diary
